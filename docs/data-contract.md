...
| part_voice | manifest 的两路全族：`mysekai/talk/part_voice/` 与 `sound/scenario/voice/part_voice_*` | 整包全解。piapro 型角色专属；装载按 UnitType 门控，人类角色没有包是真源构造，不是缺口。过门说话者装载时**两侧各取一份**（scenario 侧与 mysekai 侧同名资产、各自路径），故两路同族；更老的 `sound/scenario/part_voice/` 路径对话链不点名，不入族。manifest 标 `isBuiltin` 的包不在下发路径上（在安装包本体里），缺席时以该理由具名，不记「未在解密树」。 |
...
- `corpus.json` 是**累积账本**，并账语义与 `loop.json` 对齐：本次跑的计数累加进既有账，具名条目（failed / 缺包包名 / uncovered cue）按名合并（本次的读数胜出），本次分母没问的族整段保留原账——账本描述盘上语料的现状，而非最后一次跑。重跑同一份语料会把它的 `requested` / `requestedCues` 再累加一次（`succeeded` / `decodedCues` 不会：盘上已有就不重复解码）。逐族 `requested` / `succeeded` / `failed`、talk voice 的 cue 级覆盖、part_voice 包对语料 `partvoice_` cue 的应答（`requested` / `answered` 记最近一份语料，`missing` 是历次跑发现的无应答具名累积，**盘上有包应答即剔出账**，应答按盘上全部 part_voice 流现算）。与 `loop.json` 同规：**只写工具名与文件名，不写本机目录**。
- `partvoice.json`（`partvoice-routes` 命令产）是**说话者/家具 → 变体语音包**的路由表：participants 键为 characterUnitId（宇宙=名册 `characters.json`，键序=名册序），过 UnitType 门（角色归属 unit 为 piapro）的行带 character2d 身份与**两份包名**，门拒的行只带 `skip` 理由；fixtures 键为 mysekaiFixtureId（宇宙=mysekaiFixtureCharacter2ds 全表），带单参包名（不拼 unit、仅 mysekai 侧）。包名为扁平形（`/` 折成 `__`），与 `loop.json` 包键同构——消费侧拿它直查流表。
...
**材质 extras 的 keyword 状态**（2026-09-08 起）：每个导出材质的 `extras` 带三键——`validKeywords` / `invalidKeywords` 是 Unity 序列化字符串数组的**原样**（含空数组：空也是状态，不是缺。999 包 1805 材质里 valid 空 637、invalid 空 560、两者全空 554——后者是 shadow pass 材质的真形），`shaderKeywords` 是**解析形** = valid ∪ invalid、valid 在前（即 Unity `Material.shaderKeywords` 属性的语义）。名字刻意**不叫** `keywords`：site 与角色域的 `keywords` 字段只装 valid 半边，重名会诱使消费方把并集当 valid 集。**本游戏序列化里没有 `m_ShaderKeywords` 键**（1805 材质逐一验证为 0；Unity 2021.2 起该状态拆进 valid/invalid 两数组——早前记作 `m_ShaderKeywords` 的那份状态指的就是这里）。当前内容 distinct：valid 17 种（`_DISABLE_DITHER` 1039 / `_RECEIVE_SHADOWS_OFF` 1037 / `_USE_ALPHA_CLIP` 738 / `_ENABLE_MODULE_REFLECTION` 19 / 其余 13 种零星）、invalid 56 种（多为工具生成的 `_ENABLE_*_ON` / `_SHOW_*_ON` 残留）。导出的 glb 材质实例 1862 > 包内 1805：glb 自包含，依赖包的材质随引用多写 57 次。keyword→shader 变体编译语义（multi_compile）不在此契约内——导出的只是开关联状态，1805/1805 已与解密包源 Material 逐项核对相等。
...
site/
  ...
  harvest.json                  采集物件族的连接视图：61 包的状态、几何、视图契约与 master 联接
  scenes/<站点名>/
...
采集物件（树、石头、植物、宝箱、掉落物）是**运行时刷到采集站里的实体**，不是烤进场景的几何——它们在 `field/object` 路径下被站点提取当作 `props/` 的一员照常导出，几何与材质都在各包目录里。`harvest.json` 是这一族的**连接视图**：它不重读任何 bundle，只把同一次提取产出的 61 份包文档接成消费方要的那一张表。

每包一条，带**三态状态**（`exported` / `no-mesh` / `failed`，与家具几何索引用同一套词，两族一条规则可读）、glb 路径（相对本文件）、网格与顶点计数、shader 族名、动画片段名、粒子发射器数，以及**视图契约**：包里那个序列化字段带 `mysekaiSiteHarvestFixtureType` 的行为就是该物件的视图，其 `radius` / `mysekaiSiteHarvestFixtureType` / `collisionType` / `isRareObject` **逐字照抄**。识别走**数据不走类名**——按字段路由，提取器没见过的类只要带这个字段就照样被捡起来，而不是落进默认桶。**不带该字段的包是模型或特效伴生件，不是可采集物**：实测 61 包里 37 带视图、24 不带，后者按 `viewReason` 说明，不猜。

两个 master 联接，**双向都报**。`mysekaiSiteHarvestFixtures` 行用 `assetbundleName`（路径叶名）指包：本快照 42 行**全部**命中盘上 61 包（覆盖 36 个不同叶名，石头等稀有档一行一名多行）。`mysekaiMaterials` 行用 `modelAssetbundleName` 指的是**掉落物模型**，是另一群东西——一根原木的模型不是那棵可采的树：62 行里 36 行带模型名、全部命中。九个包**两张表都不点名**（生日同伴件、蓝图与记录掉落、纸飞机）：其中五个被同伴包声明为依赖，四个独立存在；它们按 `packagesWithoutMasterRow` 单列，不与「master 点了盘上没有的包」这一相反方向混在一个「未匹配」数里——两个方向在本快照都是零，字段仍然保留。其余三张族表（不可用点位、生日刷新、新手分组）**不含包名**，只报在与不在及行数，不给它们答不上的联接。不给 master 时，几何与视图契约照常写出，两个联接按缺失报告并给理由——与放置表同一条规则。

实测规模：61 包全部 `exported`（123 个 prefab 根、0 无网格、0 打不开）、贴图与材质随包、22 段动画曲线。
...
裁片 `sprites/<图集名>/<sprite名>.png` **不是矩形裁剪**：读器自己的 sprite 解码器（解图集指针、撤销打包旋转、对 tight 打包按 sprite 网格裁形）出什么就存什么，本模块不重做这套变换。**没有留下 `Sprite` 对象的 sprite 不裁**，`cropReason` 写明——没有网格就没有 tight 打包可解，猜一个矩形等于烤进一张错图。图集整图在 `textures/`（仅解调用方点名要的图集，本快照 51 个图集盘点、9 个解码、10 张整图：CommonAtlas 两张 2048×2048 ASTC 4x4、TalkBalloonAtlas 与 AreaAtlas 各一张 1024×1024、CheerfulCarnivalAtlas 两张、MysekaiAtlas 一张 1024×1024、ResultAtlas 与 VirtualLiveAtlas 各一张 2048×2048 ASTC 6x6、ScenarioAtlas 一张 128×512 ASTC 6x6）；其余图集照样有盘点行（含尺寸），事后补一张裁片不需要重跑发现。本快照 57 张裁片，含按钮 `btn_r30_wh`（80×80，九宫四边 38）、方向三角 `balloon_direction_triangle_wh`（40×20）、底图 `bg_base_r30_wh`（70×70，九宫 34）、TalkBalloonAtlas 的 `balloon_announce`/thinking/cry 一族、**对话窗消费的两件**——尾标 `icon_pageForward_gn`（50×44；CommonAtlas 里另有同名孪生一并裁出，消费面认 ScenarioAtlas 那张）与面板后暗带 `bg_story_adv`（4×399，九宫左边 2）——以及 balloon 前缀族在其余五图集的余量（AreaAtlas 2、CheerfulCarnivalAtlas 5、MysekaiAtlas 9、ResultAtlas 2、VirtualLiveAtlas 6）：`cropReason=undecoded` 的行已清零。

两个具名事实，读数时别误读：**MenuAtlas 出货就是空的**（0 打包名、0 贴图），而对话窗 EndSign 的星标节点按名向它要 `icon_pageForward_2`——该 sprite 在 player data 全库（所有序列化文件的 Sprite 对象逐个查过）不存在，星标真身不在数据里，属上游缺口不属提取器；tight 打包的裁片尺寸等于**网格真形**而非总是作者 `m_Rect`：57 张里 55 张与 `m_Rect` 逐项相等，两张例外都是名为 `balloon_busy` 的条目（MysekaiAtlas/VirtualLiveAtlas，tight 位为真）——透明边在打包时被裁，解码出的 114×148 才是实际不透明形状，作者 116×152 仍原样在账。
