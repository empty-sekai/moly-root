...
|---|---|---|
| talk voices | the `voice_` cues of the corpus `--talks` names (either shape this repo's talk extractors write: talks grouped under `units`, or a flat `talks` list) | **Named by cue, never extracted by package.** A cue minus its `voice_` prefix minus the trailing line/variant pair names the talk script, and the script names its own package `mysekai/talk/voice/<script>`; only the cues the corpus names are decoded — other cues inside the same archive stay undecoded on purpose. |
...
| part voices | the manifest's whole family on **two paths**: `mysekai/talk/part_voice/` and `sound/scenario/voice/part_voice_*` | Whole archive. These belong to piapro-type characters; loading is gated on UnitType, so a human character having no package is the truth of the construction, not a gap. A gate-passing speaker loads **one package per side** (the same asset name under each path), so the two paths are one family; an older `sound/scenario/part_voice/` path exists but the talk chain never names it, and it is not part of the family. A package the manifest marks `isBuiltin` is not on the download path at all (it ships inside the player build): asked for, and its absence is named for that reason rather than "not in the decrypted tree". |
...
- **Packages no corpus names are not extracted**: the manifest lists 3995 talk-voice packages and one corpus names only part of them; pointing `--talks` at another corpus names another consumer's share, and a package already on disk is never decoded twice.
...
- `corpus.json` is an **accumulating ledger**, merged the way `loop.json` is: this run's counts add to the previous ones, named entries (failed packages, missing package names, uncovered cues) merge by name with this run's reading winning, and a family this run asked nothing about keeps its previous summary — the ledger describes the corpus on disk, not the last run. Running the same corpus again re-adds its `requested` / `requestedCues` (`succeeded` / `decodedCues` are not re-added: what is on disk is not decoded twice). Per family `requested` / `succeeded` / `failed`, cue-level talk-voice coverage, and whether the part-voice packages answer the corpus's `partvoice_` cues (`requested` / `answered` describe the latest corpus; `missing` accumulates every name any run has found unanswered **until a package on disk answers it**, answered computed against all part-voice streams on disk). Same rule as `loop.json`: **tool and file names only, never a machine's directory layout**.
- `partvoice.json` (written by the `partvoice-routes` command) is the **speaker / fixture → variant-voice package** routing table: `participants` keys are characterUnitIds (the universe is the roster `characters.json`, keys in roster order), a gate-passing row carries the character2d identity and **both package names**, and a gated row carries only its `skip` reason; `fixtures` keys are mysekaiFixtureIds (the universe is the whole mysekaiFixtureCharacter2ds table) carrying the single-parameter package name (no unit suffix, mysekai side only). Package names are flattened (`/` folded to `__`), the same keys `loop.json` uses — a consumer looks streams up with them verbatim.
...
**Material extras carry the shader-keyword state** (since 2026-09-08): every exported material's `extras` holds three keys — `validKeywords` / `invalidKeywords` are the Unity serialized string arrays **verbatim** (empty arrays included: empty is a state, not a gap. Across the 1805 materials of the 999 packages, 637 have an empty valid array, 560 an empty invalid one, and 554 are empty on both — that last shape is what the shadow-pass materials really ship), and `shaderKeywords` is the **parsed form** = valid ∪ invalid, valid first (the semantics of Unity's `Material.shaderKeywords` property). The name is deliberately **not** `keywords`: the site and character domains export a `keywords` field holding only the valid half, and reusing the name would tempt a consumer into reading the union as the valid set. **This game's serialization has no `m_ShaderKeywords` key at all** (verified 0 across all 1805 materials; since Unity 2021.2 the state is split into the valid/invalid arrays — what was earlier recorded under the name `m_ShaderKeywords` is this state). Current-content distinct counts: 17 valid keywords (`_DISABLE_DITHER` 1039 / `_RECEIVE_SHADOWS_OFF` 1037 / `_USE_ALPHA_CLIP` 738 / `_ENABLE_MODULE_REFLECTION` 19 / 13 more sparse), 56 invalid ones (mostly tool-generated `_ENABLE_*_ON` / `_SHOW_*_ON` leftovers). Exported glb material instances number 1862 against 1805 in-package: a glb is self-contained, so dependency-package materials are rewritten 57 extra times. Keyword-to-shader-variant compilation semantics (multi_compile) are outside this contract — what is exported is the switch state alone, and all 1805/1805 materials have been checked equal against the decrypted source Material objects.
