...
缓存按来源与快照隔离，默认冻结已缓存表；切换版本应使用版本化来源或另一缓存目录。旧版无来源记录的缓存不会被复用。实际读取的来源与 hash 写入 `masterInputs`。下载续传同样要求可核对的对象身份，错误范围或 hash 会触发干净重试；具体状态与分发验收见[完整性说明](reliability.md)。
